...
/**
 * WS Client Feeder Status Mahasiswa Module
 * 
 * @author      Yusuf Ayuba
 * @copyright   2015
 * @Url         http://jago.link
...
class Ref_status extends CI_Controller {
    ...
    public function view($offset=0)
    {
        ...
        $temp_status = $this->input->post('nm_status');
        if ($temp_status=='') {
            ...
            $filter_status = "nm_stat_mhs like '%".$temp_status."%'";
            $temp_rec = $this->feeder->getrset($this->session->userdata('token'), 
                                                            $this->tabel, $filter_status, 
                                                            $this->order, $this->limit, 
                                                            $offset
                                                         );
            $temp_count = $this->feeder->count_all($this->session->userdata('token'), $this->tabel,$filter_status);
            $temp_jml = $temp_count['result'];
            
            $data['temp_status'] = $temp_status;
        }

        //pagination
        $config['base_url'] = site_url('ref_status/view');
        ...
        tampil('ref/__view_status',$data);
    }
}

/* End of file ref_status.php */
/* Location: ./application/controllers/ref_status.php */